class BankAccount:
    ...
    def make_deposit(self, amount):
        if amount > 0:
            self.balance += amount
            print(f"Deposit is success, New Balance is : {self.balance}")
        else:
    ...
    def make_withdrawal(self, amount):
        if amount > 0:
            if self.balance - amount >= 0:
                self.balance -= amount
                print(f"{amount} Withdraw success and remianing blance is {self.balance}")
            else:
...
my_savings_account.make_withdrawal(-3000)
my_savings_account.make_withdrawal(50401)
